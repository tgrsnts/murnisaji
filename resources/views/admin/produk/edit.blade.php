...
@section('content')
    <div class="bg-white rounded-lg shadow-lg p-6">
        <h2 class="text-2xl font-bold text-red-800 mb-6">Edit Produk</h2>
        ...
        <form action="{{ route('admin.produk.update', $produk->produk_id) }}"
            ...
            <div class="mb-6">
                ...
                <textarea name="deskripsi" placeholder="Abon sapi berkualitas tinggi dengan rasa pedas yang nikmat........."
                    class="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg h-32" required>{{ old('deskripsi', $produk->deskripsi) }}</textarea>
            </div>

            <div class="mb-6">
                <label class="block text-red-800 font-medium mb-2">Harga*</label>
                <input type="number" name="harga" value="{{ old('harga', $produk->harga) }}" placeholder="50000"
            ...
            <div class="mb-6">
                ...
                <input type="number" name="stok" value="{{ old('stok', $produk->stok) }}" placeholder="100"
                    class="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg" required>
            </div>

            <div class="mb-6">
                <label class="block text-red-800 font-medium mb-2">Berat (gram)*</label>
                <input type="number" name="berat_gram" value="{{ old('berat_gram', $produk->berat_gram) }}"
            ...
            <div class="mb-6">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                    ...
                    <div id="previewWrapper" class="mt-4 {{ $produk->gambar ? '' : 'hidden' }}">
                        @if ($produk->gambar)
                            <img id="previewImage" src="{{ asset('storage/' . $produk->gambar) }}" alt="Preview"
                                class="w-40 h-40 object-cover mx-auto rounded-lg">
                        @else
                            <img id="previewImage" src="" alt="Preview"
                                class="w-40 h-40 object-cover mx-auto rounded-lg hidden">
                        @endif
                ...
                <p class="text-gray-500 text-xs mt-1">
                    Foto saat ini akan dipertahankan jika tidak ada foto baru yang diunggah
                </p>
            </div>
