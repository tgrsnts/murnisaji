                        ...
                        <div class="flex justify-between items-start gap-4">
                            ...
                            @if ($transaction->status === 'PENDING')
                                <form method="POST" action="{{ route('payment.createSnap', $transaction->transaksi_id) }}">
                                    @csrf
                                    <button type="submit" class="bg-[#7A1F1F] text-white px-4 py-2 rounded-lg hover:bg-[#5A0F0F] transition text-sm font-medium">
                            ...
                            @foreach ($transaction->items as $item)
                                <div class="flex justify-between items-start gap-3">
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-gray-600">{{ $item->quantity }} x Rp{{ number_format($item->harga_saat_beli, 0, ',', '.') }}</p>
                                    </div>
                                    <p class="font-semibold text-gray-900">Rp{{ number_format($item->quantity * $item->harga_saat_beli, 0, ',', '.') }}</p>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-200 p-6">
                        ...
                        <div class="space-y-3 text-sm">
                            ...
                            <div>
                                ...
                                <p class="font-medium text-gray-900">{{ $transaction->detail }}, {{ $transaction->kecamatan }}, {{ $transaction->kabupaten }}, {{ $transaction->provinsi }} {{ $transaction->kodepos }}</p>
                            </div>
                            <div>
